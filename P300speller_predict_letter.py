...
'''
A stepwise linear model is a type of linear regression model where the selection of predictor variables (features) is done in a stepwise manner, typically through a combination of forward selection, backward elimination, or both. The goal is to build a regression model that includes only the most relevant variables, thus simplifying the model and potentially improving its interpretability and performance.

There are two main types of stepwise regression procedures:

Forward Selection:

The forward selection method starts with an empty model and iteratively adds the most statistically significant predictor variable at each step until no more variables meet the specified criteria for inclusion.
Backward Elimination:

The backward elimination method starts with a model containing all predictor variables and iteratively removes the least statistically significant variable at each step until no more variables meet the specified criteria for removal.
